...
def cal_qc_client(bfile_path: str, output_path: str, snp_list: List[str],
...
def calculate_allele_freq_hwe(bfile_path: str, output_path: str, snp_list: List[str]):
    extract_arg = f'--extract {output_path}.common_snp_list' if len(snp_list) > 0 else ''
...
def filter_snp (
    ...
    obs_summary = pd.DataFrame(
        {
            "ID": snp_id,
            "MISSING": obs_missing_rate,
            "HWE": obs_hwe_pval,
            "MAF": obs_maf
        }
    )

    obs_summary.loc[obs_summary.MAF>0.5, "MAF"] = 1 - obs_summary.loc[obs_summary.MAF>0.5].MAF
